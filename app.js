...
if(layer){const operationalLayers=Object.values(m._layers).filter(x=>x instanceof L.Marker||x instanceof L.Circle);for(const e of d.eventos||[]){if(!Number.isFinite(e.coordenadas?.lat)||!Number.isFinite(e.coordenadas?.lon))continue;const icon=L.divIcon({className:'linked-marker',html:'<span aria-hidden="true"></span>',iconSize:[8,8],iconAnchor:[4,4]});const mk=L.marker([e.coordenadas.lat,e.coordenadas.lon],{keyboard:true,title:e.titulo,icon}).addTo(layer).bindTooltip('<strong>'+esc(e.titulo)+'</strong><br>'+esc(e.coordenadas.precisao||'Localizacao exata'));mk.on('click',()=>detail(e));markers.set(e.id,mk)}layer.remove();const controls=document.createElement('div');controls.className='map-data-controls';controls.innerHTML='<label><input type="checkbox" id="layer-48h" checked> Noticias 48h</label><label><input type="checkbox" id="layer-30d" checked> Historico 30d</label><button type="button" id="map-overview">Visao geral</button><span>Sem localizacao suficiente: item sem marcador.</span>';document.querySelector('#operational-map').before(controls);const toggle=()=>{const on=document.querySelector('#layer-48h').checked||document.querySelector('#layer-30d').checked;operationalLayers.forEach(x=>on?(!m.hasLayer(x)&&m.addLayer(x)):(m.hasLayer(x)&&m.removeLayer(x)))};document.querySelector('#layer-48h').onchange=toggle;document.querySelector('#layer-30d').onchange=toggle;const bounds=L.featureGroup([...markers.values()]).getBounds();document.querySelector('#map-overview').onclick=()=>bounds.isValid()&&m.fitBounds(bounds.pad(.8));}
